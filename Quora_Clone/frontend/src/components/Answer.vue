<template>
  <div>
    <p class="text-muted">
      <strong style="color: crimson;">{{ answer.author }}</strong> &#8901;
      {{ answer.created_at }}
    </p>
    <p>{{ answer.body }}</p>
    <div v-if="isAnswerAuthor">
      ...
      <button @click="triggerDeleteAnswer" class="btn btn-sm btn-outline-dark">
        Delete
      </button>
    </div>
    <div v-else>
...
export default {
  ...
  methods: {
    toggleLike() {
      this.userLikeAnswer === false ? this.LikeAnswer() : this.unLikeAnswer();
    },
    LikeAnswer() {
